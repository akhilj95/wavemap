...
bool rosMsgToMap(const wavemap_msgs_ros2::msg::Map& msg, MapBase::Ptr& map) {
  ...
  LOG(WARNING) << "Conversion of the requested map ROS msg to a wavemap map "
                 "is not yet supported.";
  map = nullptr;
  return false;
}

void mapToRosMsg(const HashedBlocks& map, wavemap_msgs_ros2::msg::HashedBlocks& msg) {
  ProfilerZoneScoped;
...
void mapToRosMsg(const WaveletOctree& map, wavemap_msgs_ros2::msg::WaveletOctree& msg) {
  ProfilerZoneScoped;
  // Serialize the map and data structure's metadata
  msg.min_cell_width = map.getMinCellWidth();
  msg.min_log_odds = map.getMinLogOdds();
...
void mapToRosMsg(
    const HashedWaveletOctree& map, wavemap_msgs_ros2::msg::HashedWaveletOctree& msg,
