#ifndef WAVEMAP_ROS_ROS2_INPUTS_POINTCLOUD_TOPIC_INPUT_H_
#define WAVEMAP_ROS_ROS2_INPUTS_POINTCLOUD_TOPIC_INPUT_H_
...
#include "wavemap_ros_ros2/utils/pointcloud_undistorter.h"
...
class PointcloudTopicInput : public RosInputBase {
 public:
  ...
  void callback(const sensor_msgs::msg::PointCloud2& pointcloud_msg);
#ifdef LIVOX_AVAILABLE
  void callback(const livox_ros_driver2::msg::CustomMsg& pointcloud_msg);
#endif
 ...
 private:
  ...
  // Type-erased: registerCallback() picks the message type at runtime from the
  // config, so the subscription cannot be a
  // rclcpp::Subscription<ConcreteMsg>::SharedPtr member.
  rclcpp::SubscriptionBase::SharedPtr pointcloud_sub_;
  // NOTE: No mutex, exactly as in ROS1, where the single-threaded spinner
  //       serialized all callbacks. Preserving that requires running this node
  //       under a SingleThreadedExecutor -- see app/ros_server.cc.
...
#include "wavemap_ros_ros2/inputs/impl/pointcloud_topic_input_impl.h"

#endif  // WAVEMAP_ROS_ROS2_INPUTS_POINTCLOUD_TOPIC_INPUT_H_
